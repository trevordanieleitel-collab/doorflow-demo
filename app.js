...
let state = {
  ...
  mobileManagementGuestSearch:"",
  mobileManagementGuestScope:"ALL",
  serviceControlsExpanded:false,
...
let managementGuestAnchorGeneration = 0;
let pendingManagementGuestAnchor = null;
const serviceControlsPhoneMedia = typeof window.matchMedia === "function"
  ? window.matchMedia("(max-width: 540px)")
  : null;
let serviceControlsPhoneLayout = serviceControlsPhoneMedia
  ? serviceControlsPhoneMedia.matches
...
function captureScrollPositions() {
  const ids = ["guestScrollPanel", "groupScrollPanel", "reportScrollPanel", "mobileManagementGuestList"];
  const positions = {};
  ids.forEach(id => {
...
function captureManagementGuestAnchor(guestId = "", action = "", restoreFocus = false) {
  if (state.view !== "manage" || !canManageData()) return null;
  if (window.matchMedia && !window.matchMedia("(max-width: 760px)").matches) return null;

  const panel = document.getElementById("mobileManagementGuestList");
  const cards = [...document.querySelectorAll("[data-management-guest-card][data-guest-id]")];
  if (!panel || !cards.length) return null;

  const activeControl = document.activeElement?.closest?.("[data-management-guest-action]") || null;
  const activeCard = activeControl?.closest?.("[data-management-guest-card][data-guest-id]") || null;
  const inferredGuestId = String(activeCard?.dataset.guestId || "");
  const requestedGuestId = String(guestId || inferredGuestId);
  const requestedAction = String(action || activeControl?.dataset.managementGuestAction || "");
  const shouldRestoreFocus = Boolean(
    restoreFocus
      || (!guestId && activeControl && activeCard && document.activeElement === activeControl)
  );
  const documentScroller = document.scrollingElement || document.documentElement;
  const panelScrollable = panel.scrollHeight > panel.clientHeight + 1;
  const utilityBottom = Math.max(0, document.querySelector(".df-utility-bar")?.getBoundingClientRect().bottom || 0);
  const anchorTop = panelScrollable ? panel.getBoundingClientRect().top : utilityBottom;

  let cardIndex = requestedGuestId
    ? cards.findIndex(card => String(card.dataset.guestId || "") === requestedGuestId)
    : -1;

  if (cardIndex < 0) {
    cardIndex = cards.findIndex(card => card.getBoundingClientRect().bottom > anchorTop + 1);
  }
  ...
  return {
    generation:++managementGuestAnchorGeneration,
    view:state.view,
    guestId:String(card.dataset.guestId || ""),
    requestedGuestId,
    previousGuestId:String(cards[cardIndex - 1]?.dataset.guestId || ""),
    nextGuestId:String(cards[cardIndex + 1]?.dataset.guestId || ""),
    action:requestedAction,
    restoreFocus:shouldRestoreFocus,
    cardOffset:card.getBoundingClientRect().top - anchorTop,
    documentScrollTop:Number(documentScroller?.scrollTop || 0),
    panelScrollTop:Number(panel.scrollTop || 0)
  };
}

function prepareManagementGuestAction(event, guestId, action) {
  const control = event?.currentTarget || null;
  const snapshot = captureManagementGuestAnchor(
    guestId,
    action,
    Boolean(control && document.activeElement === control)
  );

  if (snapshot) pendingManagementGuestAnchor = snapshot;
  return snapshot;
}

function restoreManagementGuestAnchor(snapshot) {
  if (!snapshot) return;

  requestAnimationFrame(() => {
    if (snapshot.generation !== managementGuestAnchorGeneration) return;

    const primaryGuestId = snapshot.requestedGuestId || snapshot.guestId;
    if (snapshot.view !== state.view) {
      const pendingIsCurrent = pendingManagementGuestAnchor?.generation === snapshot.generation;
      const deleteStillBusy = snapshot.action === "delete"
        && isActionBusy(`mobile-delete:${primaryGuestId}`);
      if (pendingIsCurrent && !deleteStillBusy) pendingManagementGuestAnchor = null;
      return;
    }
    ...
    const findCard = id => id
      ? cards.find(card => String(card.dataset.guestId || "") === String(id)) || null
      : null;
    const primaryCard = findCard(primaryGuestId);
    const card = primaryCard
      || findCard(snapshot.nextGuestId)
      || findCard(snapshot.previousGuestId)
      || findCard(snapshot.guestId);
    const documentScroller = document.scrollingElement || document.documentElement;
    const panelScrollable = panel.scrollHeight > panel.clientHeight + 1;

    if (documentScroller) documentScroller.scrollTop = snapshot.documentScrollTop;
    panel.scrollTop = snapshot.panelScrollTop;

    if (card) {
      const utilityBottom = Math.max(0, document.querySelector(".df-utility-bar")?.getBoundingClientRect().bottom || 0);
      const anchorTop = panelScrollable ? panel.getBoundingClientRect().top : utilityBottom;
      const delta = card.getBoundingClientRect().top - anchorTop - snapshot.cardOffset;

      if (Math.abs(delta) > 1) {
        if (panelScrollable) panel.scrollTop += delta;
        else if (documentScroller) documentScroller.scrollTop += delta;
      }
    }

    const pendingIsCurrent = pendingManagementGuestAnchor?.generation === snapshot.generation;
    const keepForGuestModal = pendingIsCurrent && snapshot.action === "edit" && state.modal === "guest";
    const keepForPendingDelete = pendingIsCurrent
    ...
    const focusIsUnclaimed = !activeElement
      ...
      || activeElement === document.getElementById("main-content");

    if (snapshot.restoreFocus && state.modal !== "guest" && focusIsUnclaimed) {
      const focusAction = snapshot.action === "delete" && (pendingIsCurrent || !primaryCard)
        ? "edit"
        : snapshot.action;
      const control = card && focusAction
        ? card.querySelector(`[data-management-guest-action="${focusAction}"]`)
        : null;
      const focusTarget = control || (!card ? panel : null);
      if (focusTarget) {
        try {
    ...
    if (pendingIsCurrent && !keepForGuestModal && !keepForPendingDelete) {
      pendingManagementGuestAnchor = null;
    }
  });
}

function captureLiveRosterAnchor(guestId = "", action = "", restoreFocus = false) {
  if (state.view !== "door" && state.view !== "tabletDoor") return null;

  const cards = [...document.querySelectorAll("[data-live-roster-card][data-guest-id]")];
  if (!cards.length) return null;

  const requestedGuestId = String(guestId || "");
  const documentScroller = document.scrollingElement || document.documentElement;
  const panel = state.view === "door" ? document.getElementById("guestScrollPanel") : null;
  const panelScrollable = Boolean(panel && panel.scrollHeight > panel.clientHeight + 1);
  const utilityBottom = Math.max(0, document.querySelector(".df-utility-bar")?.getBoundingClientRect().bottom || 0);
  const anchorTop = panelScrollable ? panel.getBoundingClientRect().top : utilityBottom;
  ...
  return {
    generation:++liveRosterAnchorGeneration,
    view:state.view,
    guestId:String(card.dataset.guestId || ""),
    requestedGuestId,
    previousGuestId:String(cards[cardIndex - 1]?.dataset.guestId || ""),
    ...
    restoreFocus:Boolean(restoreFocus),
    cardOffset:rect.top - anchorTop,
    documentScrollTop:Number(documentScroller?.scrollTop || 0),
    panelScrollTop:Number(panel?.scrollTop || 0)
  };
}

function prepareLiveRosterAction(event, guestId, action) {
  const control = event?.currentTarget || null;
  const snapshot = captureLiveRosterAnchor(
    guestId,
    action,
    Boolean(control && document.activeElement === control)
  );
  ...
  pendingLiveRosterAnchor = snapshot;
  queueMicrotask(() => {
    const primaryGuestId = snapshot.requestedGuestId || snapshot.guestId;
    if (pendingLiveRosterAnchor?.generation === snapshot.generation && !isGuestCheckActionBusy(primaryGuestId)) {
      pendingLiveRosterAnchor = null;
    }
  });
}

function restoreLiveRosterAnchor(snapshot) {
  if (!snapshot) return;

  requestAnimationFrame(() => {
    if (snapshot.generation !== liveRosterAnchorGeneration || snapshot.view !== state.view) return;
    ...
    const findCard = id => id
      ...
      : null;
    const primaryGuestId = snapshot.requestedGuestId || snapshot.guestId;
    const card = findCard(primaryGuestId)
      || findCard(snapshot.nextGuestId)
      || findCard(snapshot.previousGuestId)
      || findCard(snapshot.guestId);
    const documentScroller = document.scrollingElement || document.documentElement;
    ...
    const pendingIsCurrent = pendingLiveRosterAnchor?.generation === snapshot.generation;
    const actionSettled = !isGuestCheckActionBusy(primaryGuestId);
    const activeElement = document.activeElement;
    const focusIsUnclaimed = !activeElement
      || activeElement === document.body
      || activeElement === document.documentElement
    ...
    if (snapshot.restoreFocus && focusIsUnclaimed && pendingIsCurrent && actionSettled && card && String(card.dataset.guestId || "") === primaryGuestId) {
      const actionOrder = snapshot.action === "undo"
        ? ["undo", "checkin", "toggle"]
        : snapshot.action === "toggle"
          ? ["toggle", "checkin", "undo"]
          : ["checkin", "undo", "toggle"];
      const control = actionOrder
        .map(name => card.querySelector(`[data-roster-action="${name}"]:not([disabled])`))
        .find(Boolean);
    ...
    if (pendingIsCurrent && actionSettled) pendingLiveRosterAnchor = null;
  });
}
...
function captureTransientUiState() {
  return {
    ...
    managementGuest:pendingManagementGuestAnchor?.view === state.view
      ? pendingManagementGuestAnchor
      : captureManagementGuestAnchor(),
    liveRoster:pendingLiveRosterAnchor?.view === state.view
      ? pendingLiveRosterAnchor
...
function restoreTransientUiState(ui) {
  ...
  restoreManagementGuestAnchor(ui?.managementGuest);
  restoreLiveRosterAnchor(ui?.liveRoster);
}

function resetManagementSessionUiState() {
  state.mobileManagementGuestSearch = "";
  state.mobileManagementGuestScope = "ALL";
  state.serviceControlsExpanded = false;
  pendingManagementGuestAnchor = null;
  managementGuestAnchorGeneration += 1;
}
...
function restoreModalReturnFocus(target, managementSnapshot = null) {
  ...
  if (!element && managementSnapshot?.restoreFocus) {
    const cards = [...document.querySelectorAll("[data-management-guest-card][data-guest-id]")];
    const findCard = id => id
      ? cards.find(card => String(card.dataset.guestId || "") === String(id)) || null
      : null;
    const primaryGuestId = managementSnapshot.requestedGuestId || managementSnapshot.guestId;
    const card = findCard(primaryGuestId)
      || findCard(managementSnapshot.nextGuestId)
      || findCard(managementSnapshot.previousGuestId)
      || findCard(managementSnapshot.guestId);
    element = card?.querySelector('[data-management-guest-action="edit"]')
      || document.getElementById("mobileManagementGuestList");
...
function syncModalFocus(opening) {
  ...
  if (modalFocusOpen) {
    ...
    const managementSnapshot = pendingManagementGuestAnchor?.action === "edit"
      ? pendingManagementGuestAnchor
      : null;
    modalFocusOpen = false;
    modalReturnFocus = null;
    requestAnimationFrame(() => restoreModalReturnFocus(target, managementSnapshot));
  }
}
...
function hasPendingGuestCheckActions() {
  return Object.entries(state.actionBusy || {}).some(([key, busy]) => (
    Boolean(busy) && (key.startsWith("checkin:") || key.startsWith("undo:"))
  ));
}

function markGuestCheckStateChanged() {
  guestCheckStateVersion += 1;
  return guestCheckStateVersion;
}

function beginLiveDataSnapshot() {
  return {
    sequence:++liveDataLoadSequence,
    guestCheckStateVersion,
    guestCheckActionsPending:hasPendingGuestCheckActions()
  };
}

function shouldApplyLiveDataSnapshot(snapshot) {
  return Boolean(
    snapshot
    && snapshot.sequence === liveDataLoadSequence
    && snapshot.guestCheckStateVersion === guestCheckStateVersion
    && !snapshot.guestCheckActionsPending
...
async function loadDataForDate(dateString) {
  return runDb("Loading live data", async () => {
    const liveDataSnapshot = beginLiveDataSnapshot();
    ...
    if (!shouldApplyLiveDataSnapshot(liveDataSnapshot)) {
      if (liveDataSnapshot.sequence === liveDataLoadSequence) {
        state.loading = false;
        schedulePendingSync("guest check-in state changed during refresh");
      }
      return;
    }
...
async function checkInOneGuest(id) {
  ...
  try {
    ...
    activeDoorFlowAction = hasPendingGuestCheckActions();
    lastDoorFlowActionAt = Date.now();
...
function resolvedMobileManagementGuestScope() {
  const scope = String(state.mobileManagementGuestScope || "ALL");
  if (scope === "GENERAL") return scope;
  if (scope.startsWith("GROUP:") && state.groups.some(group => group.id === scope.slice(6))) return scope;
  return "ALL";
}

function mobileManagementGuests() {
  const scope = resolvedMobileManagementGuestScope();
  const general = generalGroup();
  const terms = searchTerms(state.mobileManagementGuestSearch);
  const rows = state.guests.filter(guest => {
    if (scope === "GENERAL" && guest.group_id !== general?.id) return false;
    if (scope.startsWith("GROUP:") && guest.group_id !== scope.slice(6)) return false;
    return matchesSearchTerms(`${guest.first_name || ""} ${guest.last_name || ""}`, terms);
  });
...
function renderMobileManagementGuestScopeOptions() {
  const scope = resolvedMobileManagementGuestScope();
  return `
    <option value="ALL" ${scope === "ALL" ? "selected" : ""}>All guests</option>
    <option value="GENERAL" ${scope === "GENERAL" ? "selected" : ""}>General Guest List</option>
    ${specificGroups().map(group => `<option value="GROUP:${group.id}" ${scope === `GROUP:${group.id}` ? "selected" : ""}>${esc(group.name)}</option>`).join("")}
  `;
}

function renderMobileManagementGuestRows(guests = mobileManagementGuests()) {
  if (!state.guests.length) {
    return `<div class="mobile-management-guest-empty" role="status">No guests have been added for this service date.</div>`;
  }

  if (!guests.length) {
    return `<div class="mobile-management-guest-empty" role="status">No guests match this search and list filter.</div>`;
  }
  ...
  return guests.map(guest => {
    ...
    const checked = guestChecked(guest);
    const remaining = guestRemaining(guest);
    const deleteBusy = isActionBusy(`mobile-delete:${guest.id}`);

    return `
      <article class="mobile-management-guest-card" data-management-guest-card data-guest-id="${esc(guest.id)}">
        <div class="mobile-management-guest-copy">
          <h3>${esc(guestDisplayName(guest))}</h3>
          <p>${esc(groupNameForGuest(guest))} <span aria-hidden="true">&middot;</span> ${esc(guest.guest_type || "Guest")}</p>
          <p>Allowed ${total} <span aria-hidden="true">&middot;</span> ${checked} checked in <span aria-hidden="true">&middot;</span> ${remaining} remaining</p>
          ${isLateAdd(guest) ? `<p class="mobile-management-guest-approval ${guest.late_add_approved_by ? "approved" : "pending"}">${guest.late_add_approved_by ? `Late Add &middot; Approved by ${esc(guest.late_add_approved_by)}` : "Needs Approval"}</p>` : ""}
        </div>
        <div class="mobile-management-guest-actions">
          <button id="mobileManagementGuestEdit-${esc(guest.id)}" type="button" class="btn secondary" data-management-guest-action="edit" onclick="prepareManagementGuestAction(event,'${guest.id}','edit');openGuestModal('${guest.id}')">Edit</button>
          <button id="mobileManagementGuestDelete-${esc(guest.id)}" type="button" class="btn danger" data-management-guest-action="delete" onclick="deleteMobileManagementGuest(event,'${guest.id}')" ${deleteBusy ? "disabled" : ""}>${deleteBusy ? "Deleting..." : "Delete"}</button>
        </div>
      </article>
    `;
  }).join("");
...
function renderMobileManageGuests() {
  if (!canManageData()) return "";

  const guests = mobileManagementGuests();
  return `
    <section id="mobileManageGuests" class="mobile-manager-card mobile-management-guests" aria-labelledby="mobile-management-guests-title">
      <div class="mobile-manager-title-row">
        <div>
          <h2 id="mobile-management-guests-title">Manage Guests</h2>
          <p>View, edit, or remove names for the selected service date.</p>
        </div>
        <strong id="mobileManagementGuestCount" class="mobile-management-guest-count">${guests.length}/${state.guests.length}</strong>
      </div>

      <div class="mobile-management-guest-tools">
        <div>
          <label for="mobileManagementGuestSearch">Search names</label>
          <input id="mobileManagementGuestSearch" type="search" value="${esc(state.mobileManagementGuestSearch)}" placeholder="Search guest names..." oninput="updateMobileManagementGuestSearch(this.value)" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" />
        </div>
        <div>
          <label for="mobileManagementGuestScope">List or party</label>
          <select id="mobileManagementGuestScope" onchange="setMobileManagementGuestScope(this.value)">
            ${renderMobileManagementGuestScopeOptions()}
          </select>
        </div>
      </div>

      <div id="mobileManagementGuestList" class="mobile-management-guest-list" role="region" aria-label="Guests for ${esc(state.activeDay)} ${esc(state.activeDate)}" tabindex="0">
        ${renderMobileManagementGuestRows(guests)}
      </div>
    </section>
  `;
}

function refreshMobileManagementGuestSurface() {
  const panel = document.getElementById("mobileManagementGuestList");
  if (!panel) return false;

  const anchor = pendingManagementGuestAnchor?.view === state.view
    ? pendingManagementGuestAnchor
    : captureManagementGuestAnchor();
  const guests = mobileManagementGuests();
  const count = document.getElementById("mobileManagementGuestCount");
  const scope = document.getElementById("mobileManagementGuestScope");

  if (count) count.textContent = `${guests.length}/${state.guests.length}`;
  if (scope) {
    scope.innerHTML = renderMobileManagementGuestScopeOptions();
    scope.value = resolvedMobileManagementGuestScope();
  }
  panel.innerHTML = renderMobileManagementGuestRows(guests);
  restoreManagementGuestAnchor(anchor);
  return true;
}

function updateMobileManagementGuestSearch(value) {
  state.mobileManagementGuestSearch = String(value || "");
  refreshMobileManagementGuestSurface();
}

function setMobileManagementGuestScope(value) {
  state.mobileManagementGuestScope = String(value || "ALL");
  refreshMobileManagementGuestSurface();
}

async function deleteMobileManagementGuest(event, id) {
  const actionKey = `mobile-delete:${id}`;
  if (isActionBusy(actionKey)) return;

  setActionBusy(actionKey, true);
  const snapshot = prepareManagementGuestAction(event, id, "delete");
  let failed = false;
  ...
  try {
    ...
    setActionBusy(actionKey, false);
    const guestStillExists = state.guests.some(guest => guest.id === id);
    if (
      pendingManagementGuestAnchor?.generation === snapshot?.generation
        && ((guestStillExists && !failed) || state.view !== snapshot?.view)
    ) {
      pendingManagementGuestAnchor = null;
    }
    if (failed && guestStillExists && state.view === snapshot?.view) {
      refreshMobileManagementGuestSurface();
    }
  }
}
    ...
    <div class="mobile-manager-view">
      <section class="mobile-manager-header">
        <div class="mobile-manager-header-top">
          ${renderBrandBlock({ title:"Manager Mode", subtitle:`${venueContext.compactLabel} / ${venueContext.sharedGuestListLabel}`, variant:"dark", compact:true })}
        </div>
  ...
  if (!notes.length) {
    return `
      <section class="card df-door-shift-notes df-door-shift-notes--empty" aria-labelledby="door-shift-notes-title">
        <header class="df-door-section-heading df-door-shift-brief-row">
          <div><h2 id="door-shift-notes-title">Shift Brief</h2></div>
          <p class="df-door-shift-empty-summary" role="status">No notes</p>
        </header>
      </section>
    `;
  }
...
function renderCloseOutReportModal() {
  ...
  const canKeepPrintSectionTogether = (rows, maxRows, fields) => {
    const rowLengths = rows.map(row => fields
      .map(field => String(row?.[field] ?? ""))
      .join(" ")
      .trim()
      .length);
    return rows.length <= maxRows
      && rowLengths.every(length => length <= 180)
      && rowLengths.reduce((total, length) => total + length, 0) <= 480;
  };
  const keepGroupsTogether = canKeepPrintSectionTogether(report.groups, 4, ["group_name", "group_type", "host_name", "table_location"]);
  const keepLateAddsTogether = canKeepPrintSectionTogether(lateAdds, 4, ["display_name", "group_name", "approved_by", "reason", "added_by"]);
  const keepNoShowsTogether = canKeepPrintSectionTogether(noShows, 4, ["display_name", "group_name", "guest_type", "notes"]);
  const keepShiftNotesTogether = canKeepPrintSectionTogether(shiftNotes, 3, ["category", "priority", "note_text", "created_by_name"]);
    ...
    <div class="modal-backdrop df-admin-modal-backdrop df-closeout-report-backdrop">
      <article class="modal df-admin-modal df-closeout-modal df-closeout-report" role="dialog" aria-modal="true" aria-labelledby="closeout-report-title" aria-describedby="closeout-report-context" tabindex="-1">
        ...
        <div class="df-closeout-report__body">
          ...
          <section class="df-closeout-report__section df-closeout-report__section--groups${keepGroupsTogether ? " df-closeout-report__section--print-short" : ""}" aria-labelledby="closeout-groups-title">
          ...
          <section class="df-closeout-report__section df-closeout-report__section--late-adds${keepLateAddsTogether ? " df-closeout-report__section--print-short" : ""}" aria-labelledby="closeout-late-adds-title">
          ...
          <section class="df-closeout-report__section df-closeout-report__section--no-shows${keepNoShowsTogether ? " df-closeout-report__section--print-short" : ""}" aria-labelledby="closeout-no-shows-title">
          ...
          <section class="df-closeout-report__section df-closeout-report__section--notes${keepShiftNotesTogether ? " df-closeout-report__section--print-short" : ""}" aria-labelledby="closeout-notes-title">
          ...
          <section class="df-closeout-report__section df-closeout-report__section--activity" aria-labelledby="closeout-activity-title">
...
function currentSyncPresentation() {
  const status = state.error ? "Error" : state.loading ? "Syncing" : (state.syncStatus || "Live");
  const statusKey = String(status).toLowerCase();
  const last = formatClock(state.lastSyncAt);
  const message = state.error
    ? state.error
    : state.loading
      ? "Refreshing the active service day."
      : (state.syncMessage || "Guest and check-in data sync across devices.");
  const tone = state.error || statusKey.includes("error") || statusKey.includes("failed") || statusKey.includes("offline") || statusKey.includes("signed out")
    ? "bad"
    : state.loading || statusKey.includes("saving") || statusKey.includes("syncing") || statusKey.includes("pending") || statusKey.includes("polling") || statusKey.includes("reconnect") || statusKey.includes("connecting")
      ? "warn"
      : syncClassName();

  return { status, last, message, tone };
}

function formatServiceControlsDate(dateString) {
  if (!isValidISODate(dateString)) return String(dateString || "Date unavailable");
  const date = new Date(`${dateString}T12:00:00`);
  if (Number.isNaN(date.getTime())) return String(dateString || "Date unavailable");
  return date.toLocaleDateString([], { month:"short", day:"numeric", year:"numeric" });
}

function serviceControlsAccessibleLabel(expanded = state.serviceControlsExpanded) {
  const { status } = currentSyncPresentation();
  const action = expanded ? "Hide service controls" : "Show service controls";
  const day = String(state.activeDay || dayNameFromDate(state.activeDate) || "Day").slice(0, 3);
  const date = formatServiceControlsDate(state.activeDate);
  return `${action}. ${day} · ${date} · ${status}.`;
}

function renderServiceControlsSummary() {
  const { status, tone } = currentSyncPresentation();
  const day = String(state.activeDay || dayNameFromDate(state.activeDate) || "Day").slice(0, 3);
  const date = formatServiceControlsDate(state.activeDate);
  return `
    <span id="df-service-controls-summary" class="df-service-controls-summary">
      <span class="df-service-controls-summary__date">${esc(day)} <span aria-hidden="true">&middot;</span> ${esc(date)}</span>
      <span class="df-service-controls-summary__status ${tone}"><span class="sync-dot" aria-hidden="true"></span>${esc(status)}</span>
    </span>
  `;
}

function refreshServiceControlsSummary() {
  const button = document.getElementById("df-service-controls-toggle");
  const summary = document.getElementById("df-service-controls-summary");
  if (button) button.setAttribute("aria-label", serviceControlsAccessibleLabel());
  if (!summary) return Boolean(button);
  summary.outerHTML = renderServiceControlsSummary().trim();
  return true;
}

function syncServiceControlsDisclosure() {
  const expanded = Boolean(state.serviceControlsExpanded);
  const shell = document.querySelector(".df-shell-service");
  const button = document.getElementById("df-service-controls-toggle");

  if (shell) {
    shell.dataset.serviceControlsExpanded = expanded ? "true" : "false";
    shell.dataset.serviceControlsMobileLayout = serviceControlsPhoneLayout ? "true" : "false";
  }
  if (button) {
    const label = expanded ? "Hide service controls" : "Show service controls";
    button.setAttribute("aria-expanded", expanded ? "true" : "false");
    button.setAttribute("aria-label", serviceControlsAccessibleLabel(expanded));
    button.setAttribute("title", label);
  }
  refreshServiceControlsSummary();
}

function focusServiceControlsToggleBeforeHide() {
  const button = document.getElementById("df-service-controls-toggle");
  const controls = document.getElementById("df-service-controls");
  if (!controls?.contains(document.activeElement) || !button) return false;
...
function syncServiceControlsResponsiveLayout(matches = serviceControlsPhoneMedia?.matches) {
  const nextPhoneLayout = Boolean(matches);
  if (nextPhoneLayout && !serviceControlsPhoneLayout && !state.serviceControlsExpanded) {
    focusServiceControlsToggleBeforeHide();
  }

  serviceControlsPhoneLayout = nextPhoneLayout;
  const shell = document.querySelector(".df-shell-service");
  if (shell) shell.dataset.serviceControlsMobileLayout = nextPhoneLayout ? "true" : "false";
}

function handleServiceControlsBreakpointChange(event) {
  syncServiceControlsResponsiveLayout(event.matches);
}

function toggleServiceControls() {
  const nextExpanded = !state.serviceControlsExpanded;

  if (!nextExpanded) focusServiceControlsToggleBeforeHide();

  state.serviceControlsExpanded = nextExpanded;
  syncServiceControlsDisclosure();
}

function renderSyncPill(surface = "pill") {
  const { status, last, message, tone } = currentSyncPresentation();

  if (surface === "notice") {
    return `<div class="notice df-door-system-state df-door-live-sync-notice ${tone}" role="${tone === "bad" ? "alert" : "status"}"><strong>${esc(status)}</strong><span>${esc(message)}</span></div>`;
  }

  return `<span class="pill sync-pill ${tone}" title="${esc(message)}"><span class="sync-dot"></span>${esc(status)} · Updated ${esc(last)}</span>`;
}
  ...
  return `
    <header class="df-utility-bar">
      <div class="df-utility-inner">
        ...
        <div class="df-shell-service" data-service-controls-expanded="${state.serviceControlsExpanded ? "true" : "false"}" data-service-controls-mobile-layout="${serviceControlsPhoneLayout ? "true" : "false"}" aria-label="Service context">
          <button id="df-service-controls-toggle" class="df-service-controls-toggle" type="button" aria-controls="df-service-controls" aria-expanded="${state.serviceControlsExpanded ? "true" : "false"}" aria-label="${esc(serviceControlsAccessibleLabel())}" title="${state.serviceControlsExpanded ? "Hide" : "Show"} service controls" onclick="toggleServiceControls()">
            ${renderServiceControlsSummary()}
            <span class="df-service-controls-chevron" aria-hidden="true">&#8964;</span>
          </button>
          <div id="df-service-controls" class="df-service-controls">
            <div class="df-service-context-row">
            ...
            <div class="df-service-sync-row">
              ${renderSyncPill()}
              <button type="button" class="btn secondary small df-refresh-button" onclick="manualRefreshData()" aria-label="Refresh live data" title="Refresh live data"><span class="df-refresh-icon" aria-hidden="true">RF</span><span class="df-refresh-label">Refresh</span></button>
            </div>
          </div>
        </div>
  ...
  return `
    <div class="guest-list">
      ${guests.map(guest => {
        ...
        return `
          <div class="guest-row df-door-guest-row ${fullyIn ? "checked is-complete" : checked > 0 ? "is-partial" : "is-pending"}" data-live-roster-card data-guest-id="${esc(guest.id)}">
            <button type="button" class="guest-check df-door-guest-check" data-roster-action="toggle" onclick="prepareLiveRosterAction(event,'${guest.id}','toggle');toggleGuest('${guest.id}')" aria-label="Toggle check-in for ${esc(DOORFLOW_LIVE_GUEST_PRESENTATION.displayName(guest))}" ${guestBusy ? "disabled" : ""}>${fullyIn ? "✓" : checked > 0 ? checked : ""}</button>
            ...
            <div class="row-actions df-door-guest-actions">
              ${checkInBusy ? `<button type="button" class="btn green small df-door-checkin-action" data-roster-action="checkin" disabled>Saving...</button>` : remaining > 0 ? `<button type="button" class="btn green small df-door-checkin-action" data-roster-action="checkin" onclick="prepareLiveRosterAction(event,'${guest.id}','checkin');checkInOneGuest('${guest.id}')" ${guestBusy ? "disabled" : ""}>Check In 1</button>` : `<span class="badge in df-door-status df-door-status--complete">Fully In</span>`}
              ${undoBusy ? `<button type="button" class="btn secondary small df-door-undo-action" data-roster-action="undo" disabled>Saving...</button>` : checked > 0 ? `<button type="button" class="btn secondary small df-door-undo-action" data-roster-action="undo" onclick="prepareLiveRosterAction(event,'${guest.id}','undo');undoOneGuest('${guest.id}')" ${guestBusy ? "disabled" : ""}>Undo 1</button>` : ""}
              ${showActions ? `
...
function refreshSyncStatusSurface() {
  let touched = refreshServiceControlsSummary();
  ...
  const liveNotice = document.querySelector(".df-door-live-sync-notice");
  if (liveNotice) {
    const template = document.createElement("template");
    template.innerHTML = renderSyncPill("notice");
    const next = template.content.firstElementChild;
    if (next) {
      liveNotice.replaceWith(next);
      touched = true;
    }
  }
...
function refreshVisibleGuestSurface() {
  const rosterAnchor = pendingLiveRosterAnchor?.view === state.view
    ? pendingLiveRosterAnchor
    : captureLiveRosterAnchor();
  const guests = visibleGuests();
  let touched = false;
  ...
  syncGuestFilterControls();
  syncSortControls();
  restoreLiveRosterAnchor(rosterAnchor);
...
function refreshLiveSurfaces() {
  ...
  const touchedMobileManagementGuests = refreshMobileManagementGuestSurface();
  const touchedGroups = refreshGroupListSurface();
  const touchedSelectedGroup = refreshSelectedGroupPanelSurface();
  return touchedSync || touchedStats || touchedGuests || touchedMobileManagementGuests || touchedGroups || touchedSelectedGroup;
}
...
function renderTabletGuestCards(guests) {
  return guests.length ? guests.map(guest => {
    ...
    return `
      <article class="tablet-guest-card df-tablet-guest-card ${fullyIn ? "checked is-complete" : checked > 0 ? "is-partial" : "is-pending"}" data-live-roster-card data-guest-id="${esc(guest.id)}" aria-label="${esc(DOORFLOW_LIVE_GUEST_PRESENTATION.displayName(guest))}, ${fullyIn ? "fully checked in" : `${remaining} remaining`}">
        ...
        <div class="tablet-actions df-tablet-guest-actions">
          ${checkInBusy ? `<button type="button" class="btn green tablet-check-btn df-door-checkin-action" data-roster-action="checkin" disabled>Saving...</button>` : remaining > 0 ? `<button type="button" class="btn green tablet-check-btn df-door-checkin-action" data-roster-action="checkin" onclick="prepareLiveRosterAction(event,'${guest.id}','checkin');checkInOneGuest('${guest.id}')" ${guestBusy ? "disabled" : ""}>Check In 1</button>` : `<button type="button" class="btn green tablet-check-btn df-door-checkin-action" data-roster-action="checkin" disabled>Fully In</button>`}
          ${undoBusy ? `<button type="button" class="btn secondary tablet-undo-btn df-door-undo-action" data-roster-action="undo" disabled>Saving...</button>` : checked > 0 ? `<button type="button" class="btn secondary tablet-undo-btn df-door-undo-action" data-roster-action="undo" onclick="prepareLiveRosterAction(event,'${guest.id}','undo');undoOneGuest('${guest.id}')" ${guestBusy ? "disabled" : ""}>Undo 1</button>` : `<button type="button" class="btn secondary tablet-undo-btn df-door-undo-action" data-roster-action="undo" disabled>Undo 1</button>`}
        </div>
      </article>
    `;
...
function renderApp() {
  ...
  if (state.view === "tabletDoor") content = `<div class="stack df-tablet-door-view">${renderShiftNotesForDoorStaff()}${renderTabletDoorMode()}</div>`;
      ...
      <div class="df-shell-content">
        ...
        <main id="main-content" class="df-main-content ${isLiveServiceView ? "df-live-service-content df-door-theme" : "df-admin-theme"}" tabindex="-1">
          <div class="page${isLiveServiceView ? " df-door-page" : ""}">
            ${state.error && !isLiveServiceView ? `<div class="notice redbox df-admin-state-inline" role="alert"><strong>Error:</strong> ${esc(state.error)}</div>` : ""}
            ${isLiveServiceView ? renderSyncPill("notice") : `
              <details class="notice greenbox df-admin-state-inline df-database-mode-disclosure">
                <summary><strong>Live database mode</strong></summary>
                <p>Guests and check-ins are stored in Supabase and sync across devices.</p>
              </details>
            `}
            ${content}
          </div>
...
if (serviceControlsPhoneMedia?.addEventListener) {
  serviceControlsPhoneMedia.addEventListener("change", handleServiceControlsBreakpointChange);
} else {
  serviceControlsPhoneMedia?.addListener?.(handleServiceControlsBreakpointChange);
}
