      ...
      <article class="modal df-admin-modal df-closeout-modal df-closeout-report" role="dialog" aria-modal="true" aria-labelledby="closeout-report-title" aria-describedby="closeout-report-context" tabindex="-1">
        <header class="df-closeout-report__header">
          <div class="df-closeout-report__identity">
            <span class="df-closeout-report__monogram" aria-hidden="true">${esc(shellInitials(report.venue, "V"))}</span>
            <div class="df-closeout-report__heading">
              ...
              <p id="closeout-report-context" class="df-closeout-report__context"><strong>${esc(report.venue)}</strong><span aria-hidden="true">&middot;</span><span>${esc(report.day_name)} ${esc(report.service_date)}</span></p>
      ...
      ${guests.map(guest => {
        ...
        return `
          <div class="guest-row ${fullyIn ? "checked" : ""}">
            <button type="button" class="guest-check" onclick="toggleGuest('${guest.id}')" ${guestBusy ? "disabled" : ""}>${fullyIn ? "✓" : checked > 0 ? checked : ""}</button>
            ...
            <div class="row-actions">
              ${checkInBusy ? `<button type="button" class="btn green small" disabled>Saving...</button>` : remaining > 0 ? `<button type="button" class="btn green small" onclick="checkInOneGuest('${guest.id}')" ${guestBusy ? "disabled" : ""}>Check In 1</button>` : `<span class="badge in">Fully In</span>`}
              ${undoBusy ? `<button type="button" class="btn secondary small" disabled>Saving...</button>` : checked > 0 ? `<button type="button" class="btn secondary small" onclick="undoOneGuest('${guest.id}')" ${guestBusy ? "disabled" : ""}>Undo 1</button>` : ""}
              ${showActions ? `
...
function renderSelectedGroupPanel() {
  ...
  if (!group) {
    return `
      <div id="selectedGroupPanel" class="card">
    ...
    return `
      <div class="tablet-guest-card ${fullyIn ? "checked" : ""}">
        ...
        <div class="tablet-actions">
          ${checkInBusy ? `<button type="button" class="btn green tablet-check-btn" disabled>Saving...</button>` : remaining > 0 ? `<button type="button" class="btn green tablet-check-btn" onclick="checkInOneGuest('${guest.id}')" ${guestBusy ? "disabled" : ""}>Check In 1</button>` : `<button type="button" class="btn green tablet-check-btn" disabled>Fully In</button>`}
          ${undoBusy ? `<button type="button" class="btn secondary tablet-undo-btn" disabled>Saving...</button>` : checked > 0 ? `<button type="button" class="btn secondary tablet-undo-btn" onclick="undoOneGuest('${guest.id}')" ${guestBusy ? "disabled" : ""}>Undo 1</button>` : `<button type="button" class="btn secondary tablet-undo-btn" disabled>Undo 1</button>`}
        </div>
      </div>
    `;
  ...
  return `
    ...
    <div class="stack manage-desktop-view df-admin-page df-management-page">
      ${renderAdminPageHeader({
        ...
        meta:[
          { label:"Venue", value:state.venue?.name || "Venue" },
    ...
    <div class="df-app-shell ${isLiveServiceView ? "df-shell-live" : "df-shell-admin"}">
      ...
      <div class="df-shell-content">
        ...
        <main id="main-content" class="df-main-content ${isLiveServiceView ? "df-live-service-content" : "df-admin-theme"}" tabindex="-1">
          <div class="page">
            ${state.error ? `<div class="notice redbox df-admin-state-inline" role="alert"><strong>Error:</strong> ${esc(state.error)}</div>` : ""}
            <div class="notice greenbox df-admin-state-inline" role="status"><strong>Live database mode:</strong> guests and check-ins are stored in Supabase and sync across devices.</div>
            ${content}
          </div>
        </main>
      </div>
