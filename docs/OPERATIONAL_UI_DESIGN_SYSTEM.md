...
## Phase 8.1 mobile management stabilization

### Mobile Manage Guests

At Management phone widths, `Manage Guests` derives its rows directly from the already-loaded `state.guests` and `state.groups` for the active service date. `canManageData()` gates the renderer, while `openGuestModal(id)` and `deleteGuest(id)` retain the existing permission checks, form validation, confirmation, payloads, and post-write reconciliation. Admin and Manager roles therefore receive the section through the current authorization model; Door and Viewer roles do not receive a new route or action surface.

The compact record hierarchy is guest name, list or party, guest type, allowed count, checked-in and remaining count, and a Late Add or Needs Approval state only when the record carries that operational state. Edit and destructive Delete are always-visible, text-labeled controls with 44px minimum targets. Search matches guest names in memory, and the list scope supports All guests, General Guest List, and the loaded party/group records without a query per keystroke or a second guest collection. Dedicated mobile search and scope fields leave Door search, filters, and selected-list behavior unchanged.

The bounded mobile list participates in transient scroll capture and uses an isolated requested/next/previous guest anchor. Edit holds the anchor through the existing modal and focus-return flow, including a logical-neighbor fallback when an edited name leaves the active search. Delete delegates once to the existing handler, keeps its anchor through intermediate realtime paints, and restores the nearest retained record or the focusable list region when no result remains. Ordinary partial refreshes also preserve a focused Edit/Delete control when that record remains. The action generation prevents an older render or an action settled on another tab from overriding the latest mobile management action. These are presentation safeguards only and do not change guest or group mutation logic.

### Single service-date authority and phone disclosure

Management no longer renders a desktop Service Date card, a mobile Change Service Date card, or a Management header Service Date label. The shared utility bar remains the sole visible Day and native Service Date selector. `setActiveDay`, `setActiveDate`, `useTodayDate`, service-day loading, realtime resubscription, and current date state remain intact.

At 540px and below, the shared service region starts collapsed and exposes one `button` with `aria-expanded`, `aria-controls`, a Show/Hide action name that includes the visible day/date/status summary, and a visible chevron. Its summary is built from the active day, formatted active date, and the same resolved connection state used by the sync pill; Error, Saving, Syncing, Pending, Polling, Reconnecting, Offline, and Live remain text-backed rather than color-only. Expansion reveals the unchanged Day selector, native Service Date input, sync status, and Refresh control. The choice is stored only in runtime state, survives tab renders during the current authenticated session, resets when that session ends, and does not use browser or database storage.

The disclosure updates its DOM in place. Before either a manual collapse or a desktop-to-phone breakpoint collapse, focus inside the controlled panel moves to the disclosure with `preventScroll`; expansion does not move focus into the fields. A synchronized mobile-layout data attribute delays breakpoint hiding until that focus transfer is complete. The expanded Day and Service Date controls retain separate phone rows, the P6.9 safe-area spacing, and the P6.10 20px native calendar overlay gutter. The Phase 8.1 stylesheet URL uses `/doorflow-operational-theme.css?v=p8.1`.

`sw.js` remains on the reviewed `doorflow-cache-v30` implementation. Its explicit inventory keeps the unversioned stylesheet path and its allowlist compares `url.pathname`, so the p8.1 query URL remains eligible without changing lifecycle, navigation, request-exclusion, activation, or cleanup behavior. Static smoke checks confirm source contracts, protected hashes, runtime/mirror parity, and unchanged SQL/service-worker/manifest files; they do not prove responsive geometry, authenticated role behavior, live mutations, focus or scroll outcomes, realtime delivery, or installed-PWA lifecycle behavior. Those remain owner acceptance work with approved non-production records.
